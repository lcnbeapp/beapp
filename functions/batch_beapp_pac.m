...
%     newcrossf from eeglab
         ...
         if grp_proc_info_in.pac_save_all_reports == 1 ||  ...
          ...
          if exist(strcat(src_dir{1},filesep,grp_proc_info_in.beapp_fname_all{curr_file}),'file')
            ...
            if grp_proc_info_in.pac_calc_zscores
                ...
                amp_dist = NaN(grp_proc_info_in.pac_high_fq_res, grp_proc_info_in.pac_low_fq_res,18,numsegs,size(file_proc_info.beapp_indx{1,1},2),201); 
            else
                amp_dist = NaN(grp_proc_info_in.pac_high_fq_res, grp_proc_info_in.pac_low_fq_res,18,numsegs,size(file_proc_info.beapp_indx{1,1},2)); 
            end
    ...
    %                     caxis([.02 .065])
          ...
          %  amp_dist = squeeze(nanmean(amp_dist,4));
            ...
            if grp_proc_info_in.pac_calc_zscores
                amp_dist = reshape(amp_dist,z([1 2 3 5 6]));
            else
                if size(amp_dist,5)>1
                    amp_dist = reshape(amp_dist,z([1 2 3 5]));
                end
            end
          
            [new_zscore_comod, rawmi_comod] = beapp_calc_mi_zscore(amp_dist,grp_proc_info_in.pac_calc_zscores);
            ...
            %% save output .mat and excel files
             ...
             filename = erase(file_proc_info.beapp_fname{1},'.mat');
             if grp_proc_info_in.slid_win_on == 1
                save(strcat(filename,'_pac_results','.mat'),'comodulogram','comodulogram_column_headers','comodulogram_row_headers','comodulogram_third_dim_headers','comodulogram_4th_dim_headers','amp_dist','new_zscore_comod','rawmi_comod','file_proc_info');
             else
                save(strcat(filename,'_pac_results','.mat'),'comodulogram','comodulogram_column_headers','comodulogram_row_headers','comodulogram_third_dim_headers','amp_dist','new_zscore_comod','rawmi_comod','file_proc_info');
             end
