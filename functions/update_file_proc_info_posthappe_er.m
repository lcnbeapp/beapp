...
for condition_idx = 1:size(file_proc_info.evt_conditions_being_analyzed,1)
    file_proc_info.evt_conditions_being_analyzed.Num_Segs_Pre_Rej(condition_idx) = dataQCTab(1).dataQC(find(strcmp({dataQCTab.dataQCnames},char(strcat('Number_',file_proc_info.evt_conditions_being_analyzed.Condition_Name(condition_idx),'_Segs_Pre-Seg_Rej')))));
    file_proc_info.evt_conditions_being_analyzed.Num_Segs_Post_Rej(condition_idx) = dataQCTab(1).dataQC(find(strcmp({dataQCTab.dataQCnames},char(strcat('Number_',file_proc_info.evt_conditions_being_analyzed.Condition_Name(condition_idx),'_Segs_Post-Seg_Rej')))));
end
end