...
class Comodulogram(object):
    ...
    def __init__(self, fs, low_fq_range, low_fq_width=2., high_fq_range='auto',
                 ...
                 extract_params=dict(), low_fq_width_2=4.0, n_jobs=1, compute_shifts=1, shifts=0):
        self.fs = fs
    ...
    def fit(self, low_sig, high_sig=None, mask=None):
        ...
        if self.method in STANDARD_PAC_METRICS:
            ...
            for this_mask in mask:
                phase, amp, phase_bins, amp_dist = _comodulogram(self, filtered_low, filtered_high,
                                        this_mask, filtered_low_2)
                #originally: results = _comodulogram...
                #all_results.append(results)
                all_amps.append(amp)
                all_phases.append(phase)
                all_phase_bins.append(phase_bins)
                all_amp_dist.append(amp_dist)
        ...
        if not multiple_masks:
            ...
            self.amp = all_amps
            self.phase = all_phases
            self.phase_bins = all_phase_bins
            self.amp_dist = all_amp_dist
            return self
...
def _comodulogram(estimator, filtered_low, filtered_high, mask,
    ...
    comod_norm_amp_list = np.zeros((N_BINS_TORT, n_shifts, n_low, n_high))
    for i in range(n_low):
        # preproces the phase array
        if estimator.method == 'tort':
            n_bins = N_BINS_TORT
            phase_bins = np.linspace(-np.pi, np.pi, n_bins + 1) #og!!!
            # phase_bins = np.linspace(-np.pi, 0, n_bins/2 + 1)
            # get the indices of the bins to which each value in input belongs
            phase_preprocessed = np.digitize(filtered_low[i], phase_bins) - 1 #could also keep n_bins the same
        ...
        norm_amp_list = Parallel(n_jobs=estimator.n_jobs)(delayed_func( #og: mi_list = ...
            ...
            ax_special=estimator.ax_special, estimator=estimator) for j in range(n_high))
        #mi_list = np.array(mi_list).reshape(n_high, n_shifts).T
        norm_amp_list = np.array(norm_amp_list).reshape(n_high, n_shifts, N_BINS_TORT).T
        phase_bins = estimator.phase_bins

        #comod_list[:, i, :] = mi_list
        comod_norm_amp_list[:, :, i, :] = norm_amp_list
        if estimator.progress_bar:
            estimator.progress_bar.update_with_increment_value(1)
    return filtered_low, filtered_high, phase_bins, norm_amp_list #originally: return comod_list
    #return comod_list
...
def _one_modulation_index(amplitude, phase_preprocessed, norm_a, method, shift,
    ...
    # Modulation index as in [Canolty & al 2006]
        ...
        #where you can get amplitude & phase
    ...
    # Modulation index as in [Tort & al 2010]
        ...
        n_bins = N_BINS_TORT
        amplitude_dist = np.ones(n_bins)  # og: ones(n_bins)  # default is 1 to avoid log(0)
       ...
       # amplitude_dist /= np.sum(amplitude_dist)
        ...
        #for half phase: np.linspace(-np.pi,,0,n_bins+1)
        estimator.phase_bins = 0.5 * (phase_bins[:-1] + phase_bins[1:]) / np.pi * 180
        #estimator.phase_bins = phase_bins
        estimator.amplitude_dist = amplitude_dist
        if ax_special is not None and shift == 0:
