...
switch current_sub_panel
    case 'psd'
        ...
        if grp_proc_info.beapp_toggle_mods{'psd','Module_Xls_Out_On'}
            ...
            adv_out_mod_button_geometry(end+1:end+2) = {1};
            adv_out_mod_ver_geometry(end+1:end+2) = [1,5];
        end
        
    case 'itpc'
            panel_title = 'Advanced ITPC Settings';
        adv_out_mod_button_list = [{{'style','checkbox','string', 'Include Max ITPC in Excel Report?','tag','itpc_xls_max_on',...
            'Value',grp_proc_info.beapp_itpc_xlsout_mx_on}},...
            {{'style','checkbox','string', 'Include Mean ITPC in Excel Report?','tag','itpc_xls_mean_on',...
            'Value',grp_proc_info.beapp_itpc_xlsout_av_on}}];
        adv_out_mod_button_geometry = {1 1};
        adv_out_mod_ver_geometry = [1 1];
