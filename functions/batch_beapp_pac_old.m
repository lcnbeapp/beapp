...
for curr_file=1:length(grp_proc_info_in.beapp_fname_all)
     ...
     if grp_proc_info_in.pac_save_all_reports == 1 ||  ...
      ...
      if exist(strcat(src_dir{1},filesep,grp_proc_info_in.beapp_fname_all{curr_file}),'file')
        ...
        if grp_proc_info_in.slid_win_on == 1
            ...
            comodulogram =  NaN(grp_proc_info_in.pac_high_fq_res, grp_proc_info_in.pac_low_fq_res,max_n_channels); %num chans hard coded temporarily
        end
        phase = -3.1;
        idx = 1;
        while phase < 3.1
         ...
         if grp_proc_info_in.pac_set_num_segs == 0 || size(curr_eeg,3) > (grp_proc_info_in.pac_num_segs-1)
            ...
            for chan = 1:max_n_channels
                if ismember(chan,file_proc_info.beapp_indx{1,1}) && ~(chan==file_proc_info.net_ref_elec_rnum) ...%%should work if beapp_indx{1,1} only contains channels that haven't been removed -- CHECK
                    ...
                    comodulogram_third_dim_headers{chan} = strcat('Channel # ',num2str(chan));
                    curr_chan_comodulogram = NaN(grp_proc_info_in.pac_high_fq_res, grp_proc_info_in.pac_low_fq_res,size(curr_eeg,3));
                    curr_chan_phase_amp = NaN(size(curr_eeg,3),size(phase_range,2));
                    
                    %randomly set segments to run, or run all of them
                    if grp_proc_info_in.pac_set_num_segs == 1
                        segments_torun = randsample(size(curr_eeg,3),grp_proc_info_in.pac_num_segs); %TEMP
                    else
                        ...
                        if grp_proc_info_in.slid_win_on == 1
                            ...
                            curr_chan_comodulogram = NaN(grp_proc_info_in.pac_high_fq_res, grp_proc_info_in.pac_low_fq_res,size(curr_eeg,3),num_slides);
                             for time = 1:num_slides
                                ...
                                curr_chan_comodulogram(:,:,seg,time) = beapp_calc_comod(curr_time_sig,file_proc_info.beapp_srate,low_fq_range,high_fq_range,...
                                    grp_proc_info_in.pac_method,grp_proc_info_in.pac_low_fq_width,grp_proc_info_in.pac_high_fq_width,grp_proc_info_in.pac_low_fq_res,grp_proc_info_in.pac_high_fq_res);
                                comodulogram_4th_dim_headers(1,time) = ((sample_start+sample_end)/2)*.001;
                             end
                        else
                             curr_chan_comodulogram(:,:,seg) = beapp_calc_comod(signal,file_proc_info.beapp_srate,low_fq_range,high_fq_range,...
                                    grp_proc_info_in.pac_method,grp_proc_info_in.pac_low_fq_width,grp_proc_info_in.pac_high_fq_width,grp_proc_info_in.pac_low_fq_res,grp_proc_info_in.pac_high_fq_res);
                        end
                     end
                    if grp_proc_info_in.slid_win_on == 1
    ...
    %                     caxis([.02 .065])
        ...
        if grp_proc_info_in.pac_set_num_segs == 0 || size(curr_eeg,3) > (grp_proc_info_in.pac_num_segs-1)
             cd(grp_proc_info_in.beapp_toggle_mods{'pac','Module_Dir'}{1});
             filename = erase(file_proc_info.beapp_fname{1},'.mat');
             save(strcat(filename,'_pac_results','.mat'),'comodulogram','comodulogram_column_headers','comodulogram_row_headers','comodulogram_third_dim_headers','comodulogram_4th_dim_headers','all_chan_phase_amp');
