...
for curr_file=1:length(grp_proc_info_in.beapp_fname_all)
      ...
      if exist(strcat(src_dir{1},filesep,grp_proc_info_in.beapp_fname_all{curr_file}),'file')
        ...
        while phase < 3.1
         ...
         if grp_proc_info_in.pac_set_num_segs == 0 || size(curr_eeg,3) > (grp_proc_info_in.pac_num_segs-1)
            ...
            for chan = 1:max_n_channels %TEMP should be max_n_channels
                    ...
                    if grp_proc_info_in.pac_set_num_segs == 1
                     ...
                     for seg = 1:size(segments_torun,1) 
                        ...
                        if grp_proc_info_in.slid_win_on == 1
                            ...
                            [curr_chan_comodulogram(:,:,seg), curr_z_score, curr_surr_max] = beapp_calc_comod(signal,file_proc_info.beapp_srate,low_fq_range,high_fq_range,...
                                    grp_proc_info_in.pac_method,grp_proc_info_in.pac_low_fq_width,grp_proc_info_in.pac_high_fq_width,grp_proc_info_in.pac_low_fq_res,grp_proc_info_in.pac_high_fq_res,grp_proc_info_in.pac_calc_zscores);
                        end
    ...
    %                     caxis([.02 .065])
        ...
        if grp_proc_info_in.pac_set_num_segs == 0 || size(curr_eeg,3) > (grp_proc_info_in.pac_num_segs-1)
             ...
             filename = erase(file_proc_info.beapp_fname{1},'.mat');
             if grp_proc_info_in.slid_win_on == 1
                save(strcat(filename,'_pac_results','.mat'),'comodulogram','comodulogram_column_headers','comodulogram_row_headers','comodulogram_third_dim_headers','comodulogram_4th_dim_headers','all_chan_phase_amp','z_score_comod','surr_max');
             else
                 save(strcat(filename,'_pac_results','.mat'),'comodulogram','comodulogram_column_headers','comodulogram_row_headers','comodulogram_third_dim_headers','all_chan_phase_amp','z_score_comod','surr_max');
             end
