...
grp_proc_info.src_dir={'C:\Datasets\example'}; %the directory containing your source files
grp_proc_info.beapp_curr_run_tag = 'my_current_run'; % The tag you would like to append to folder names for this run. def = '' or 'NONE'. 'NONE' mutes timestamping. If not given on a rerun, a timestamp will be used. 
...
grp_proc_info.src_unique_nets={'HydroCel GSN 128 1.0'}; % def ={''} If not running HAPP-E with multiple nets, optional for speed. Required for more than one net if running HAPP-E
...
grp_proc_info.src_eeg_vname={''}; %possible variable name of the EEG data EEG_Segment1
...
grp_proc_info.win_size_in_secs = .5; % def = 1; (second)
...
grp_proc_info.evt_seg_win_start = -1.500; % def = -0.100;  start time in seconds for segments, relative to the event marker of interest (ex -0.100, 0) 
grp_proc_info.evt_seg_win_end = 2.000;  % def = .800; end time in seconds for segments, relative to the event marker of interest (ex .800, 1) 
...
grp_proc_info.evt_analysis_win_start = -0.750; % def = -0.100;  start time in seconds for analysis segments, relative to the event marker of interest (ex -0.100, 0) 
grp_proc_info.evt_analysis_win_end = 1.750;  % def = .800; end time in seconds for analysis segments, relative to the event marker of interest (ex .800, 1) 
...
grp_proc_info.evt_trial_baseline_win_start = -.750; % def = -0.100;  start time in seconds for baseline, relative to the event marker of interest (ex -0.100, 0). Must be within range you've segmented on. 
grp_proc_info.evt_trial_baseline_win_end = -.250; % def = -0.100;  start time in seconds for baseline, relative to the event marker of interest (ex -0.100, 0) 
...
grp_proc_info.bw(3,1:2)=[4,6]; %bandwidth 3
grp_proc_info.bw_name(3)={'LowTheta'}; %name of bandwidth 3
grp_proc_info.bw(4,1:2)=[6,8]; %bandwidth 4
grp_proc_info.bw_name(4)={'HighTheta'}; %name of bandwidth 4
grp_proc_info.bw(5,1:2)=[8,12]; %bandwidth 5
grp_proc_info.bw_name(5)={'Alpha'}; %name of bandwidth 5
grp_proc_info.bw(6,1:2)=[6,9]; %bandwidth 6
grp_proc_info.bw_name(6)={'LowAlpha'}; %name of bandwidth 6
grp_proc_info.bw(7,1:2)=[9,13]; %bandwidth 7
grp_proc_info.bw_name(7)={'HighAlpha'}; %name of bandwidth 7
grp_proc_info.bw(8,1:2)=[13,30]; %bandwidth 8
grp_proc_info.bw_name(8)={'Beta'}; %name of bandwidth 8
grp_proc_info.bw(9,1:2)=[30,55]; %bandwidth 8
grp_proc_info.bw_name(9)={'Gamma'}; %name of bandwidth 8
...
grp_proc_info.psd_interp_typ=3; %type of interpolation of psd 1 none, 2 linear, 3 nearest neighbor, 4 piecewise cubic spline  
...
grp_proc_info.beapp_itpc_params.min_freq = 4;
grp_proc_info.beapp_itpc_params.max_freq = 55;
grp_proc_info.beapp_itpc_params.min_cyc = 1;
grp_proc_info.beapp_itpc_params.max_cyc = 10;
