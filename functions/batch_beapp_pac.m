...
calc_btwn_chans = 0;
if calc_btwn_chans==1
    pac_between_chans(grp_proc_info_in);
elseif calc_btwn_chans==2 %%TEMP, FOR JUST GETTING PHASE SERIES
    pac_get_phase(grp_proc_info_in);
else
         ...
         if grp_proc_info_in.pac_save_all_reports == 1 ||  ...
          ...
          if exist(strcat(src_dir{1},filesep,grp_proc_info_in.beapp_fname_all{curr_file}),'file')
            ...
            if grp_proc_info_in.pac_calc_zscores
                ...
                amp_dist = NaN(grp_proc_info_in.pac_high_fq_res, grp_proc_info_in.pac_low_fq_res,18,30,18,201);
            else
                amp_dist = NaN(grp_proc_info_in.pac_high_fq_res, grp_proc_info_in.pac_low_fq_res,18,30,18); %18 = N_bins ... don't init channels to minimize variable size
            end
    %         phase = -3.1;
    %         idx = 1;
    %         while phase < 3.1
                ...
                for chan = 1:max_n_channels 
                    if ismember(chan,file_proc_info.beapp_indx{1,1}) && ~(chan==file_proc_info.net_ref_elec_rnum) ...%%should work if beapp_indx{1,1} only contains channels that haven't been removed -- CHECK
                       ...
                       %disp(strcat('Running Channel # ',num2str(chan)))
                        ...
                        if grp_proc_info_in.pac_calc_zscores
                            curr_chan_amp_dist = NaN(grp_proc_info_in.pac_high_fq_res, grp_proc_info_in.pac_low_fq_res,18,30,201); 
                        else
                            curr_chan_amp_dist = NaN(grp_proc_info_in.pac_high_fq_res, grp_proc_info_in.pac_low_fq_res,18,30);                       
                        end
    ...
    %          end
            ...
            if grp_proc_info_in.pac_set_num_segs == 0 || size(curr_eeg,3) > (grp_proc_info_in.pac_num_segs-1)
                 ...
                 filename = erase(file_proc_info.beapp_fname{1},'.mat');
                 if grp_proc_info_in.slid_win_on == 1
                    save(strcat(filename,'_pac_results','.mat'),'comodulogram','comodulogram_column_headers','comodulogram_row_headers','comodulogram_third_dim_headers','comodulogram_4th_dim_headers','amp_dist','file_proc_info');
                 else
                     save(strcat(filename,'_pac_results','.mat'),'comodulogram','comodulogram_column_headers','comodulogram_row_headers','comodulogram_third_dim_headers','amp_dist');
                 end
