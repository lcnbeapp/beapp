...
grp_proc_info.src_eeg_vname={'Category_1_Segment1','Category_1'}; %possible variable name of the EEG data EEG_Segment1
...
grp_proc_info.evt_seg_win_start = -0.800; % def = -0.100;  start time in seconds for segments, relative to the event marker of interest (ex -0.100, 0) 
...
grp_proc_info.evt_analysis_win_start = 0; % def = -0.100;  start time in seconds for analysis segments, relative to the event marker of interest (ex -0.100, 0) 
grp_proc_info.evt_analysis_win_end = 0.650;  % def = .800; end time in seconds for analysis segments, relative to the event marker of interest (ex .800, 1) 
...
grp_proc_info.bw(1,1:2)=[1,3.99]; %bandwidth 1 start and end frequencies (the first band), can have as many or as few bandwidths as the user would like
grp_proc_info.bw_name(1)={'Delta'}; %name of bandwidth 1
grp_proc_info.bw(2,1:2)=[4,7.99]; %bandwidth 2
grp_proc_info.bw_name(2)={'Theta'}; %name of bandwidth 2
grp_proc_info.bw(3,1:2)=[8,12.99]; %bandwidth 3
...
grp_proc_info.bw(6,1:2)=[30,55]; %bandwidth 6
...
grp_proc_info.pac_low_fq_res = 39; %The # of frequencies to calculate between the min and max;Ex: to calculate for frequencies 1-10, set min to 1, max to 10, and res to 10
...
grp_proc_info.pac_high_fq_res = 41; %The # of frequencies to calculate between the min and max;Ex: to calculate for frequencies 1-10, set min to 1, max to 10, and res to 10
